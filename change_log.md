...
### 2026-09-25 — Replaced the Venn diagram with ovals overlaid on the solved puzzle itself

- **Source:** Trevor didn't like the original circle Venn diagram. Several rounds of standalone mockups (rectangles, then ovals) before he redirected to the real fix: overlay the ovals directly on the existing solved puzzle row instead of a separate diagram below it, so the puzzle display and the "how it worked" reveal are the same element.
- **Finding:** the first mockup's oval positions were hand-guessed from assumed font-metric widths, and text visibly poked outside the ovals — Trevor caught it. Fixed by measuring the real rendered text via `getBoundingClientRect()` instead of guessing. That same bug briefly reappeared during the real implementation (one debug query showed oval heights of ~190px against an expected ~52px) — traced to a stale reading during testing, not the app itself; a clean reload-and-resubmit produced correct measurements every time after.
- **Why it matters:** directly answers Trevor's two complaints — the overlap-diagram-and-puzzle redundancy is gone (one element now, not two), and the "middle word pairs with both sides" idea reads clearly again because the ovals visibly overlap on the real revealed letters.
- **Change made:** removed `buildVennDiagram()`, the `#venn-diagram` div, and its CSS entirely. `renderSolved()` in `script.js` now builds two `.overlap-oval` divs positioned/sized from real `getBoundingClientRect()` measurements of the puzzle fragments and answer, laid behind a `.puzzle-row` flex container so the text paints on top. New CSS: `.overlap-oval` (translucent `var(--accent)` at 0.25 opacity, `pointer-events: none`), `.puzzle-row`, `position: relative` added to `.puzzle-display`. Verified in-browser: canonical puzzle, longest word pair (`marketplace`/`placemat`, 11 characters, no overflow), and a mobile viewport.
- **Open assumption:** oval positions are computed once at solve time and not re-measured on window resize — if the browser window resizes or rotates after solving, the ovals could drift out of alignment with the text. Not tested; low risk for a beta but worth knowing before wider release.

### 2026-09-25 — Round 2: 23 new puzzle candidates generated for expanding past 56

- **Source:** Trevor's request to build out the puzzle list further.
- **Finding:** found 19 clean new connector words (town, field, gate, post, mill, stick, fall, shot, gun, glass, wood, arm, cake, bread, milk, eye, tooth, nail, hair) with no answer or compound overlap against the shipped 56 — verified programmatically, not just by eye. Also revisited 4 of the 5 candidates cut during round 1's curation (case, stone, fish, bug) — those were cut for not being picked, not for being wrong, so they're back in the pool with that context stated plainly. The 5th (bear/teddybear) stays cut — it was the one flagged "informal/toy term" during generation, so resurfacing it without new information would just be re-litigating a call already made.
...
### 2026-09-25 — New phrase-based mechanic: 93 candidates generated, kept as a separate track

- **Source:** Trevor's request to expand beyond compound words into common two-word English phrases, using the same overlap mechanic. Clarified upfront: fixed/idiomatic phrases only (not just any plausible noun combo), and kept as a separate track rather than mixed into the compound-word rotation.
- **Finding:** generated in two passes — an initial carefully-vetted 29, then pushed for volume when asked to try for 500. Landed at 93 and stopped there (Trevor's call) rather than pad further: hub words with 4-5 genuinely fixed phrases on both sides get harder to find past the first ~25, and by hub #40+ the honest yield was 1-2 before quality started slipping into flagged territory (dated idioms, phrases that sometimes fuse into one word). Reaching 500 at the strict bar as asked wasn't realistic without lowering the bar, and that tradeoff was surfaced rather than silently resolved either way.
- **Why it matters:** first real test of whether the puzzle mechanic generalizes beyond compound words. It does, but the supply of genuinely idiomatic two-word phrases is smaller than compound words at this quality bar — a real constraint on how far this track can scale.
- **Change made:** new file `puzzle-candidates-phrases.md` — 93 rows, same table format as the compound-word list, several answer words intentionally repeated across different phrase pairs (not deduped to one-per-answer, unlike the compound track's final state). Explicitly **not** merged into `puzzle-candidates.md` or `puzzles.js` — a separate, uncurated pool per Trevor's instruction not to add to the candidates list without asking.
- **Open assumption:** whether this track should eventually enforce one-answer-per-puzzle the same way the compound track does is unresolved — flagged in the file itself, not decided. Also open: whether/how this ever merges into the daily rotation, and how many of the ~8 flagged entries (dated idioms, sometimes-fused phrases) survive a curation pass.

### 2026-09-25 — Phrase track merged into the daily rotation: content now 172 puzzles

- **Source:** Trevor's instruction to join the 93 phrase candidates with the daily rotation into one large list — resolving the "whether/how this merges" question left open in the prior entry.
- **Finding:** the merge required zero code changes. `{a, b, answer}` is already just three single words regardless of mechanic — the app never actually checks that `a+answer` fuses into one dictionary word, it only ever renders `${a} _____ ${b}` and compares the typed guess to `answer`. Confirmed this by finding the display and oval-overlay logic worked identically for a phrase puzzle (day 79, "credit"/"card"/"game") with no visual or functional difference from a compound one. Flagged the real consequence before merging, not after: 15 answer words already existed in both tracks (dog, house, table, game, night, light, time, work, show, room, hand, line, ball, door, head), and the phrase track repeats internally (card ×5, school ×5, etc.) — merging as-is means the compound track's post-round-1 "one answer per puzzle" property no longer holds anywhere in the merged list. Trevor's instruction was to merge anyway.
...
### 2026-09-25 — New feature: post-solve history modal (month calendar)

- **Source:** Trevor's request for a history popup after each puzzle, showing green/yellow/red circles by outcome. Two rounds of clarification before building: resolved a color collision (1-point solves and outright fails were both specified as "red" — split into orange vs. red), confirmed auto-show-and-dismiss behavior, and picked a real month calendar (weekday headers, date numbers, prev/next nav) over a compact contribution-style grid, via two rendered mockups.
- **Finding:** none structural — this was the first feature built directly against a pre-confirmed design rather than iterating after the fact, and it matched the mockup on the first real implementation.
- **Why it matters:** gives players a persistent sense of their own history beyond the single streak number, and surfaces "missed days" (a state that existed in the streak logic since day one but was never visible anywhere in the UI).
- **Change made:** new `#history-modal` markup in `index.html` (month nav header, weekday row, calendar grid, color-key legend, close button). `script.js`: `dateStringToDayIndex`/`dayIndexToDateString` (UTC-string-anchored, DST-safe — factored out of the existing `getDayIndex` logic rather than duplicating it) map calendar dates to day indices; `renderHistoryCalendar()` builds the grid using plain local `Date` arithmetic for weekday/month layout (a deliberately different mechanism from the day-index math — one is calendar-grid layout, the other is puzzle-content lookup, and conflating them would reintroduce the DST bug the day-index system was built to avoid). `openHistoryModal()` is called only from the submit handler's solve/fail branches, not from `render()` itself, so reloading an already-completed day doesn't re-trigger it. Month nav bounded to [launch month, today's effective month] via the debug-offset-inclusive day index. Verified in-browser end to end: all five cell states rendered correctly and distinctly in one calendar (solved on each of the three guess counts, an outright fail, and a genuinely skipped day sitting right next to each other), days before launch correctly read as neutral rather than "missed," forward and backward month navigation both worked with correct bounds, and reloading a completed day did not reopen the modal.
- **Open assumption:** no persistent way to reopen the history later in this pass (matches what was asked — solve/fail-triggered only). Not tested against a real DST transition (the mechanism is designed to be safe, consistent with the rest of the app's day-index math, but the earliest real boundary is months out from launch).

### 2026-09-25 — Reopen icon + shareable history text

- **Source:** Trevor's request to add a persistent way to reopen the history modal, and make the history shareable as text.
- **Finding:** two real things surfaced while building this, not just the two features asked for. (1) Moving the history-modal setup earlier in `DOMContentLoaded` (needed so the reopen icon still works after the not-launched/exhausted early returns) exposed a genuine CSS bug: `.history-modal-overlay { display: flex; }` has the same specificity as the browser's built-in `[hidden] { display: none; }` rule and loads after it, so the modal was showing on every page load regardless of its `hidden` attribute — caught by screenshotting a fresh load and seeing the modal open with an unpopulated grid, not by reading the CSS. (2) `getDayStatus(cellDayIndex)` was factored out of `renderHistoryCalendar()` as the single source of truth for a cell's outcome, so the new shareable-text builder and the visual grid can't drift apart from each other.
- **Why it matters:** the CSS bug would have shipped completely broken — every page load would have opened over a blank calendar. The refactor (shared status function, shared clipboard-copy helper) means the two share buttons and the two history-display surfaces can't independently drift out of sync as this gets touched again later.
- **Change made:** `index.html` — new `#history-open-button` (📅, next to the streak, always visible) and a "Share history" button + fallback-message paragraph inside the modal. `script.js` — `copyWithFeedback()`/`fallbackCopy()` promoted to top-level shared helpers (the original "Share result" button now calls the same function instead of carrying its own copy of the clipboard logic); `buildHistoryShareText()` renders the *currently viewed* month as a 🟢🟡🟠🔴⭕⬜ emoji grid. `styles.css` — added `.history-modal-overlay[hidden] { display: none; }` to fix the specificity bug, plus styling for the new button and message. Verified in-browser: reopen icon works both before and after completing today's puzzle; share button correctly falls back through to the visible "copy this text" message in this sandboxed browser (confirmed `execCommand` fails here the same way it did for the original share button); the emoji grid correctly places a solved day's color at the right calendar position.
- **Open assumption:** the ⬜ (missed/future) emoji rendered as a font-fallback glyph in this testing browser while the colored circles rendered fine — likely a font-coverage gap specific to that environment, not a data problem, but unverified on real phones/desktops. Also: sharing is scoped to whichever month is currently displayed in the modal, not the player's full history — reasonable default, but worth confirming that's the intended scope if this comes up again.

### 2026-09-25 — Renamed "missed puzzle" to "failed puzzle"; delayed the history modal by 1 second

- **Source:** Trevor's two requests — clearer legend wording (a fail is a fail, not a "miss"), and a beat to see the day's own result before the history modal covers it.
- **Finding:** none structural. "missed puzzle" only existed in one place (the legend label in `index.html`) plus one description of it in `CLAUDE.md` — no other UI string or code path referenced that phrase.
- **Why it matters:** the wording fix removes an ambiguity ("missed" was already doing double duty for "missed day" elsewhere in the same legend); the delay fixes a real UX gap — the modal was covering the solve/fail reveal (points earned, the answer on a fail, the oval overlay) before a player had a chance to see it.
- **Change made:** `index.html` legend label → "failed puzzle". `script.js` — both `openHistoryModal()` calls in the submit handler now run via `setTimeout(openHistoryModal, 1000)` instead of immediately after `render()`. Verified in-browser on both the solve and fail paths: modal confirmed still hidden (and the result message visible) at the 500ms mark, confirmed open shortly after the 1-second mark.
- **Open assumption:** none — both changes are small and fully verified.
