...
- 2026-09-25: Live at `https://tlava27.github.io/betwixt/` (repo moved from private `tlaverriere/Betwixt` to public `tlava27/betwixt` — GitHub Pages needs a paid plan to publish from a private repo). Added a beta-testing "skip to next day" control and restyled the title's "i" (upside-down, dark orange). Since then: changed the share-text icon to echo the flipped "i", switched the streak from flat +1/day to points by guess number (5/3/1) with both an inline points message and a "(?)" rules panel, relabeled "Streak" to "Points streak" in all visible UI text, replaced the original standalone Venn diagram with translucent ovals overlaid directly on the solved puzzle row, expanded compound-word content from 56 to 79 puzzles, added a second mechanic (common idiomatic two-word phrases) and merged 93 of those in as-is for **172 puzzles total**, and added a month-calendar history modal that auto-opens once after each solve/fail. Answer-word repetition across the merged content is real and accepted, not an oversight — see the Content key fact below.
