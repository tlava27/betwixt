...
- 2026-09-25: Live at `https://tlava27.github.io/betwixt/` (repo moved from private `tlaverriere/Betwixt` to public `tlava27/betwixt` — GitHub Pages needs a paid plan to publish from a private repo). Added a beta-testing "skip to next day" control and restyled the title's "i" (upside-down, dark orange). Since then: changed the share-text icon to echo the flipped "i", switched the streak from flat +1/day to points by guess number (5/3/1) with both an inline points message and a "(?)" rules panel, replaced the original standalone Venn diagram with translucent ovals overlaid directly on the solved puzzle row, expanded compound-word content from 56 to 79 puzzles, then added a second mechanic — common idiomatic two-word phrases (e.g. "credit card" / "card game" → card) using the identical data shape and rendering — generated 93 of those and merged them in as-is. **Content is now 172 puzzles total.** Answer-word repetition is now real and accepted, not an oversight — see the Content key fact below.
